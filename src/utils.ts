...
/**
 * Fetches the history of states for a list of entities from Home Assistant over a period.
 * @param hass The Home Assistant object.
 * @param entityIds The list of entity IDs to fetch history for.
 * @param daysAgo How many days ago to start the history from.
 * @returns A promise that resolves to a record mapping entity IDs to an array of state objects.
 */
export async function fetchHistoryRange(
  hass: HomeAssistant,
  entityIds: string[],
  daysAgo: number,
): Promise<Record<string, HAHistoryItem[]>> {
  if (entityIds.length === 0) return {};
  ...
  try {
    const history = await hass.callWS<Record<string, HAHistoryItem[]>>({
      type: 'history/history_during_period',
      start_time: startTime.toISOString(),
      entity_ids: entityIds,
    ...
    console.log('[SeaTempCard] fetchHistoryRange response:', history);

    return history || {};
  } catch (err) {
    console.error(`Error fetching history range for ${entityIds.join(', ')}:`, err);
    return {};
  }
}
